...
<!-- 也许我们可以在这里备注一下，译文基于 4a6d92d 版本原文 -->
## 简介
...
## 特性丰富的编程语言
当我们喜欢的语言发布新特性(Feature) 时<!-- 疑似主语缺失？ -->往往会很兴奋。我们会学习它们，并在代码里用起来。

如果特性很多，我们可能会花费半小时，通过写几行代码来试试某个功能或者另一个功能<!-- 句号 -->。。。这多少有点浪费时间。但更糟的是，**等你过一阵子再回头看，你还得重新想一遍当时的思路<!-- 原文 recreate 是不是有从头再来的意思？看到你有地方翻译为了推演，感觉这个词不错 -->！**
 
**你不仅要理解这个复杂的程序，还得搞明白：当时写代码的人，为什么会觉得用这些特性<!-- 原意似乎强调了在“彼时彼地” -->来解决问题是个好办法。** `🤯`

这些话来自 Rob Pike。

> 减少选择，就能降低认知负荷。  

编程语言的特性是没问题<!-- AI翻译没到位，“是很好的”是不是更佳？ -->的，只要这些特性之间互不干扰。 

<details>
  <summary><b>一位拥有 20 年 C++ 经验的工程师的想法 ⭐️</b></summary>
  <br>
  前几天我看我的 RSS 阅读器，发现“C++”标签下居然有三百来篇未读文章。我从去年夏天起就没读过一篇关于 C++ 的文章，而且我感觉棒极了<!-- 而且我觉得我的感觉一如既往的好！ -->！<br><br>
  我用 C++ 已经 20 年了，差不多是我人生的三分之二。我的大部分经验都花在语言的边边角角<!-- 原意似乎有“晦暗的角落”的意思，譬如 spec 中各种很少使用的特性 -->(各种未定义行为)上。这不是可复用的经验，如今要把它们全都丢下，多少有点奇怪<!-- 原意似乎有‘不安’的意思 -->。<br><br>
  ...
  你不能仅仅为一个平凡类型(trivial type) 分配内存然后直接 <code>memcpy</code> 将字节复制过去——那并不会启动对象的生命周期。这在 C++20 之前是这样的。在C++20的版本里修了，但语言的认知负荷却增加<!-- 原意有只增不减的意思 -->了。<br><br>
  认知负荷在不断增加，即便有些问题被修正了<!-- 可以考虑倒着写？即使…却在不断… -->。<!-- “毕竟我是专业人士”调整到前面？原文作者可能写得比较随意？ -->我得知道什么被修了、何时修的、修之前是什么样。毕竟我是专业人士。没错，C++ 对遗留的特性支持很好，这也意味着你<b>会<!-- 得直面 -->直面</b>那些遗留。例如上个月同事问了我一个 C++03 的行为。<code>🤯</code><br><br>
  以前有 20 种初始化方式。后来加了统一初始化语法。现在我们有 21 种<!-- 加个‘了’？ -->。顺带一提，还有谁能记住从初始化列表里选择构造函数的规则吗？好像是跟隐式转换和“信息损失最小”有关，<i>但如果</i>值在编译期是已知的，那就是另一回事了……<code>🤯</code><br><br>
...
前端的工程师在实现登录功能时，会<!-- 感觉原文有‘借助后端实现前段’的意思 -->用到后端 API。于是他们必须在脑子里临时记住这样一组映射：
...
同样的规则适用于各种“数字类型的状态码”<!-- 很好👍你从 AI 翻译中又拯救了读者一次 -->(无论在数据库里还是别处)——**优先使用自描述的字符串**。我们早就不处在需要为 640K 内存做优化的年代了。

> 人们会为 `401` 和 `403` 争论不休，依据各自的心智模型下判断。新的开发者加入后，他们又得重新推演<!-- 这个推演也很好，但下文再遇到你精妙的翻译我就不再说了 -->一遍当时的思路。也许你写了 ADR 来记录“为什么”要这样做，帮助新人理解当时的决策。但归根结底，这并不明智。我们可以大体把错误分成“用户相关”和“服务器相关”，除此之外，界线其实很模糊。 
...
“不要重复自己<!-- wikipedia:zh-cn:不要重复你自己 -->”(Do not repeat yourself, DRY)——这是作为软件工程师最早学到的原则之一。它深入骨髓，以至于我们难以容忍多出的几行重复代码。尽管它总体上是个良好且基础的准则，但过度使用会带来难以承受的认知负荷。
...
事实上，即使在单个模块内，也会出现同样的问题。你可能会过早地抽取出所谓的“通用功能”，基于一些看起来相似但长远来看根本不存在的共性<!-- 这个似乎是一个倒装 -->。这会导致产生一些不必要的抽象，难以修改或扩展。

Rob Pike 说过：

> 适度的复制比适度的依赖更好<!-- 也有这样的翻译：少量的复制好过引入新的依赖 -->。  

我们常常过于抗拒“重复造轮子”，以至于为了使用一个很小的函数，不惜引入庞大而笨重的库。而这些库我们本可以轻松自己写出来。

**所有依赖，都是你的代码。<!-- 原文似乎通过两个 your 强调 ‘从你的来，到你的去’ -->** 当你不得不翻阅某个外部库的十几层调用堆栈，只为了搞清楚哪里出了问题时(因为问题总是会发生)，是很痛苦的。
...
所有依赖，都是你的代码。** 当你不得不翻阅某个外部库的十几层调用堆栈，只为了搞清楚哪里出了问题时（因为问题总是会发生），是很痛苦的
<!-- 鄙见1：为简化代码而引入的依赖，成为了你新的枷锁。Bug 来去无踪，不可预测，当你为了定位问题的根源而不得不翻阅某个依赖库的的十几层调用栈时，你会知道什么叫做折磨。 -->
<!-- 鄙见2：你需要为你的依赖清单负责。Bug 来去无踪，不可预测，当你为了定位问题的根源而不得不翻阅某个依赖库的的十几层调用栈时，你会知道什么叫做折磨。 -->
<!-- 鄙见3：你的依赖库，也是你代码的一部分。Bug 来去无踪，不可预测，当你为了定位问题的根源而不得不翻阅某个依赖库的的十几层调用栈时，你会知道什么叫做折磨。 -->
## 框架的紧耦合
框架里有很多“魔法”(A.K.A. 奇技淫巧)。当我们过度依赖某个框架时，**会迫使后来的开发者先去学习它们<!-- 先去学习这些魔法 -->**，这可能要花上几个月。虽然框架能让我们在几天内推出 MVP，但从长远看，它们往往带来不必要的复杂性与认知负荷。

更糟糕的是，当遇到一个与架构格格不入的新需求时，框架可能会成为最大的约束。于是人们开始 fork 框架并维护自家定制版<!-- 自家是不是有些口语化，比如 ‘似有定制版’‘自有定制版’‘定制版本’ -->。想象下新人要积累多少认知负荷(也就是学会这个定制框架)才能产出价值。`🤯`

**当然，我们绝不是提倡从零开始造轮子！！**

我们可以以相对“框架无关”的方式编写代码。业务逻辑不应被放在框架内部；相反，它应该仅使用框架的组件。把框架放在核心逻辑之外，用它像用库(library)一样。这样一来，新加入的贡献者从第一天就能带来价值，而不需要先在框架的复杂细节中摸爬滚打。 

> [Why I Hate Frameworks<!-- 《为什么我讨厌“框架”》？上一版本的中译版这样翻译的 -->](https://minds.md/benji/frameworks)
...
抽象本应隐藏复杂性，但这里它只是增加了[间接层](https://fhur.me/posts/2024/thats-not-an-abstraction)。要快速定位问题、理解哪里出错了、缺了什么，通常需要沿着调用链逐步跟踪。但在这种分层架构中，层与层之间的解耦导致我们需要额外的、甚至割裂的多层调用链来找到错误点。每一条这样的调用链，都会占据我们有限的工作记忆。`🤯`<!-- 好翻译👍 只是，‘定位问题’后要换成逗号吗？‘调用链’要统一成前文用过的‘调用栈’吗？ -->

这种架构最初看起来很直观，但我们每次在项目中实践，收效都不如预期，得不偿失。最终我们回归朴素的依赖倒置原则。**不用再学什么端口/适配器，不要非必要的的横向抽象层，不制造外在认知负荷。**

<details>
  <summary><b>Coding principles and experience</b></summary> <!-- 咦，这里怎么换成了英文。‘编程信念 & 工作年限’ -->
  <img src="img/complexity.png" <!-- 也许可以添加‘随着年限的增长，从最开始只能写简单代码，经过“这里可以加个OOP，那里可以加个策略模式”，最后返璞归真又写上了最淳朴的代码’？ -->><br>
  <a href="https://twitter.com/flaviocopes"><!-- 也许可以添加 ‘By’ ？ -->@flaviocopes</a>
</details>
...
我们做过一次存储迁移，<!-- 英文似乎 ‘and that’ 有强调‘而这’之意 -->花了大约 10 个月。老系统是单线程的，因此对外暴露的事件是顺序<!-- 假如换成‘线性的’呢？或者不换好像也不错 -->的。我们的系统都依赖这种可观察到的行为。但这种行为既不在 API 的契约里，也不在代码中体现。新的分布式存储无法保证<!-- 依我个人的理解，原文有‘分布式存储没有这种特性’的意思 -->顺序，事件就会<!-- 原文似乎没有强调因果关系 -->乱序触发。我们只花了几个小时就写好了新的存储适配器(storage adapter)，这要归功于抽象化<!-- 假如说‘这的确要归功于…’ -->。**接下来的 10 个月，我们都花在处理乱序事件及其他挑战上。** 现在再说“抽象/分层能帮助我们快速更换组件”，想起来<!-- 原文‘It's now’和‘想起来’似乎有些不搭 -->有点可笑。

**既然分层架构带来了如此高的认知负荷，而未来可能没有回报，为什么要付出这样的代价？** 况且在大多数情况下，所谓“未来要替换某些核心组件”的场景根本不会发生。  

这些架构本身并非基础性的原则，而是一些主观的、带偏见的实现方式。为什么要依赖这些主观解释？我们应该遵循更基础的规则：依赖倒置原则、单一事实来源(single source of truth)、认知负荷控制、信息隐藏。业务逻辑不应该依赖数据库、UI 或框架等底层模块。我们应该能够在不依赖基础设施的前提下，为核心逻辑<!-- 这里填上一个‘编’呢？ -->写测试，这就足够了。[参与讨论](https://github.com/zakirullin/cognitive-load/discussions/24)。
...
<div align="center">
  <img src="/img/layers.png" alt="Layers" <!-- alt：正在理解软件架构的工程人员 ？--> width="400">
</div>

## 领域驱动设计(Domain-driven design, DDD)
领域驱动设计有很多优秀的理念，但也常被误解<!-- 这里也许可以倒置一下，我看上一版调换了位置，似乎更通顺 -->。有人说：“我们用 DDD 写代码”。这有点怪，因为 DDD 更多地关乎问题空间(problem space)，而非解决方案空间(solution space)。

<!-- 另外，上一版本译者作了译注，你可以看一下要不要保留 -->
<!-- TDD 我实践不多，就不看这一章其余的部分啦 -->

通用语言(ubiquitous language)、领域(domain)、限界上下文(bounded context)、聚合(aggregate)、事件风暴(event storming)——这些都属于问题空间。它们帮助我们洞悉领域、划定边界。DDD 让开发者、领域专家与业务人员能用一种统一语言高效沟通。然而，相较于关注这些问题空间的方面，我们常常强调特定的文件夹结构、服务(service)、仓库(repository) 以及其他解决方案层面的技术。 
...
## 熟悉项目的认知负荷<!-- 上一版译文使用了‘熟悉的项目中的认知负荷’ --> 

> 问题在于，**熟悉并不等于简单**。它们的 *感觉* 相似——都让人能在一个空间<!-- ‘空间’替换为‘领域’呢？AI翻译的时候似乎没太get到这个词 -->里轻松穿梭、无需太多心力——但原因完全不同。你使用的每一个“聪明”(其实是“自我炫技”<!-- 原来的放纵之意应该是‘随着心情写的意思，感觉AI翻译得也不赖’ -->)且非大家惯用的技巧，都会让别人付出额外的学习成本。一旦他们学会了，那与代码相处就没那么难了。这就是为什么你很难看出如何简化你已经熟悉的代码。我会尽量让“新来的开发者”在他们还没被环境同化之前来评审代码！  
>
...
>
> 在代码库里，没有任何“简化的力量”会自动发生。唯一能让代码变简单的，就是你自己做出的有意识的选择。简化需要付出努力，而人们往往太赶时间<!-- 上一版中‘无暇顾及’的翻译也不错 -->。
>
...
<div align="center">
  <img src="/img/mentalmodelsv15.png" alt="心智模型" <!-- alt：对比新来的开发者，对项目熟稔的人不需要承担认知成本——他们已经把相关知识内化为了长期记忆 --> width="700">
</div>

要学习的心智模型越多，新成员产出价值所需的时间就越长。  

当你为新人做项目onboarding时<!-- 不好翻译的话，上一个版本的翻译使用了‘入职培训’的译法 -->，试着衡量他们的困惑程度(结对编程会有帮助)。如果他们连续 ~40 分钟仍很困惑——那你的代码就有改进空间。  
...
<!-- 建议（连带引号）： --><!-- - “我们的架构是标准的 CRUD 应用架构，[一个基于 Postgres 的 Python 单体](https://danluu.com/simple-architectures/)” -->
...
这些问题很难追踪，人们也往往不愿正面回答<!-- 或者‘直接回答’？ -->。看看世界上最复杂且经得住时间考验的软件系统——Linux、Kubernetes、Chrome 和 Redis(见下方评论)。你不会在它们身上找到什么花哨的东西——大多“很无聊”，这恰恰是好事。

## 结论
想象一下，如果我们在第二章里推断出的结论其实并不正确。那意味着，我们刚刚否定掉的结论，以及之前章节里我们认为正确的那些结论，也可能都不成立。`🤯`  

感受到了吗？你不仅需要在文章中来回跳转才能抓住意思(浅模块！)，而且这个段落本就难懂。我们刚刚在你的脑中制造了不必要的认知负荷。**不要把这种事加诸<!-- 是不是可以加一个介词，例如‘在’ -->你的同事身上。**

<div align="center">
  <img src="/img/smartauthorv14thanksmari.png" alt="Smart author" <!-- 这里的图片和前文呼应，我理解为“再强调一次，外在负荷愈少愈好” --> width="600">
</div>
