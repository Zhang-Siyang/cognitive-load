...
如果特性很多，我们可能会为几行代码折腾半小时，只为“这个特性还是那个特性”。这多少有点浪费时间。但更糟的是，**当你回头再看时，你得重建当时的思考过程！**
 
**你不仅要理解这个复杂程序，还得理解那位程序员在现有特性集合下，为什么要用那种方式解决问题。** `🤯`
...
<details>
  ...
  我用 C++ 已经 20 年了，差不多是我人生的三分之二。我的大部分经验都花在语言最阴暗的角落（各种未定义行为）上。这不是可复用的经验，如今要把它们全都丢下，多少有点瘆人。<br><br>
  比如你能想象吗，<code>||</code> 在 <code>requires ((!P&lt;T&gt; || !Q&lt;T&gt;))</code> 和 <code>requires (!(P&lt;T&gt; || Q&lt;T&gt;))</code> 中含义不同。前者是约束析取（constraint disjunction），后者是我们熟悉的逻辑或运算符，而且它们的行为不同。<br><br>
  你不能仅仅为一个平凡类型分配内存然后直接 <code>memcpy</code> 一组字节过去——那并不会开启对象的生命周期。这在 C++20 之前是这样的。C++20 里修了，但语言的认知负荷却更大了。<br><br>
  认知负荷在不断增加，即便有些问题被修正了。我得知道什么被修了、何时修的、修之前是什么样。毕竟我是专业人士。没错，C++ 对遗留的支持很好，这也意味着你<b>会直面</b>那些遗留。例如上个月同事问了我一个 C++03 的行为。<code>🤯</code><br><br>
  以前有 20 种初始化方式。后来加了统一初始化语法。现在我们有 21 种。顺带一提，还有谁能记住从初始化列表里选择构造函数的规则吗？大概是“信息损失最小的隐式转换”，<i>但如果</i>值是编译期已知，那就……<code>🤯</code><br><br>
  <b>这些额外的认知负荷并非来自当前的业务任务，不是领域的内在复杂性。它只是因为历史原因而存在</b>（<i>外在认知负荷</i>）。<br><br>
