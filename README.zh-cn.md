...
“不要重复自己”（Do not repeat yourself, DRY）——这是作为软件工程师最早学到的原则之一。它深入骨髓，以至于我们难以容忍多出的几行重复代码。尽管它总体上是个良好且基础的准则，但过度使用会带来难以承受的认知负荷。
...
事实上，即使在单个模块内，也会出现同样的问题。你可能会过早地抽取出所谓的“通用功能”，基于一些看起来相似但长远来看根本不存在的共性。这会导致产生一些不必要的抽象，难以修改或扩展。
...
## 框架的紧耦合
框架里有很多“魔法/奇技淫巧”。当我们过度依赖某个框架时，**会迫使后来的开发者先去学习它们**。这可能要花上几个月。虽然框架能让我们在几天内推出 MVP，但从长远看，它们往往带来不必要的复杂性与认知负荷。

更糟糕的是，当遇到一个与架构格格不入的新需求时，框架可能会成为最大的约束。于是人们开始 fork 框架并维护自家定制版。想象下新人要积累多少认知负荷（也就是学会这个定制框架）才能产出价值。`🤯`

**当然，我们绝不是提倡从零开始造轮子！！**

我们可以以较“框架无关”的方式编写代码。业务逻辑不应被放在框架内部；相反，它应该仅使用框架的组件。把框架放在核心逻辑之外，用它像用库（library）一样。这样一来，新加入的贡献者从第一天就能带来价值，而不需要先在框架的复杂细节中摸爬滚打。 
